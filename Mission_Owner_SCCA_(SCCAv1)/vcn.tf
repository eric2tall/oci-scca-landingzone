...
locals {
	...
	network_firewall = {
		network_firewall_name        = "network-firewall"
			...
			ip_address_lists = {
				"workload-cidr" = ["192.168.2.0/24"]
				"vdms-cidr"     = ["192.168.1.0/24"]
					"vcn-ips" = [var.vdss_vcn_cidr_block]
			}
			security_rules = {
			"allow-workload" = {
				security_rules_action                 = "ALLOW"
				security_rules_condition_applications = []
				security_rules_condition_destinations = []
				security_rules_condition_sources      = ["workload-cidr"]
			...
			"allow-vdms" = {
				...
				security_rules_condition_applications = []
				security_rules_condition_destinations = []
				security_rules_condition_sources      = ["vdms-cidr"]
				security_rules_condition_urls         = []
			}
			"reject-all-rule" = {
